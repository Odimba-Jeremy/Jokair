...
def create_invoice():
    ...
    if data.get("subscriber_id"):
        # Appliquer la couverture de l'abonné
...
def update_billing_account():
    ...
    patient_id = to_int(data.get("patient_id"))
    amount = round(to_float(data.get("amount"), 0), 2)
    type_operation = data.get("type", "debit")  # debit = ajout dette, credit = paiement
    description = data.get("description", "")
    ...
    schemas = {
        ...
        "pharmacy_movements": """
            CREATE TABLE IF NOT EXISTS pharmacy_movements (
                id SERIAL PRIMARY KEY,
                medication_id INTEGER REFERENCES pharmacy_items(id) ON DELETE CASCADE,
        ...
        "patient_account_transactions": """
            CREATE TABLE IF NOT EXISTS patient_account_transactions (
                id SERIAL PRIMARY KEY,
                patient_id INTEGER REFERENCES patients(id) ON DELETE CASCADE,
        ...
        "subscribers": """
            CREATE TABLE IF NOT EXISTS subscribers (
                ...
                type VARCHAR(50) DEFAULT 'Mutuelle',
                coverage_rate DECIMAL(5,2) DEFAULT 80,
        ...
        "patient_lab_results": """
            CREATE TABLE IF NOT EXISTS patient_lab_results (
                id SERIAL PRIMARY KEY,
                patient_id INTEGER REFERENCES patients(id) ON DELETE CASCADE,
                test_id INTEGER REFERENCES laboratory_tests(id) ON DELETE CASCADE,
        ...
        """,
        "laboratory_stock": """
            CREATE TABLE IF NOT EXISTS laboratory_stock (
                id SERIAL PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                category VARCHAR(100) DEFAULT 'Réactif',
                quantity INTEGER DEFAULT 0,
        ...
        """,
        "laboratory_prelevements": """
            CREATE TABLE IF NOT EXISTS laboratory_prelevements (
                id SERIAL PRIMARY KEY,
                test_id INTEGER REFERENCES laboratory_tests(id) ON DELETE CASCADE,
                patient_id INTEGER REFERENCES patients(id) ON DELETE CASCADE,
                num_prelevement VARCHAR(50) NOT NULL,
                type VARCHAR(100),
                notes TEXT,
                preleve_par INTEGER,
                preleve_par_name VARCHAR(100),
                created_at TIMESTAMP DEFAULT NOW()
            )
        """
    }
