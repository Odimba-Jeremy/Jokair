...
# Redis est utilisé en production si REDIS_URL est fourni ; le cache mémoire
# conserve une exécution locale immédiate, sans dépendance supplémentaire.
...
def compress_and_measure_response(response):
    """Réduit le transfert réseau des réponses JSON sans modifier les API."""
...
def optimize_response(response):
    """Réduit le transfert réseau sans modifier le contenu fonctionnel."""
...
def hospital_patient_id(patient_id: Any) -> str:
    """Identifiant lisible et stable, sans migration de la base existante."""
...
def create_service_invoice(patient_id: int, description: str, amount: float, source: str, source_id: int = None, account_line: dict = None):
    """Facture immédiatement une prestation clinique et marque sa ligne de compte."""
    amount = round(to_float(amount), 2)
    if not patient_id or amount <= 0:
        return None
...
def generate_barcode_svg(patient_id: int, patient_name: str) -> str:
    """Génère un code-barres SVG pour un patient"""
    from datetime import datetime
    
    patient_id_str = f"PAT-{patient_id:06d}"
    now = datetime.now().strftime("%d/%m/%Y")
    
    # Génération d'un motif de code-barres simple
    bars = []
    import random
    random.seed(patient_id)
    
    # Créer un motif binaire à partir de l'ID
...
def get_patients():
    ...
    if search:
        # La recherche "001" retrouve aussi IH-USHD-001, sans ajouter de colonne
        # ni imposer une migration aux installations existantes.
...
def link_lab_result_to_patient(patient_id: int):
    ...
    # Lier le résultat au dossier patient
    lab_result = {
        "patient_id": patient_id,
...
def get_patient_barcode(patient_id: int):
    """Génère un code-barres SVG pour le patient"""
...
def regenerate_patient_barcode(patient_id: int):
    """Régénère le code-barres d'un patient (utile pour l'impression)"""
    patient = supabase.table(TABLES["patients"]).select("id,full_name").eq("id", patient_id).execute()
    if not patient.data:
        return jsonify({"error": "Patient introuvable"}), 404
...
def get_patient_queue():
    ...
    if role == "docteur":
        # Un médecin ne voit que ses patients effectivement dispatchés et encore actifs.
        ...
        # Dès le dispatch, le patient disparaît instantanément de la réception.
        rows = [row for row in rows if row.get("status") == "waiting"]
    elif role == "infirmier":
        # L'infirmier ne voit jamais une file déjà attribuée à un médecin.
...
def workflow_consultations():
    ...
    # Une consultation tarifée produit immédiatement sa facture liée au même patient.
    # Les autres frais restent regroupables par la facture finale existante.
...
def dispense_medication(item_id: int):
    ...
    try:
        with app.test_request_context(json=invoice_data):
            ...
            # Appel à la route billing/grouped
...
def create_grouped_invoice():
    ...
    # Si source pharmacie, déduire du stock et ajouter au compte
...
def merge_invoices():
    ...
    for inv_id in invoice_ids:
        ...
        # Marquer l'ancienne facture comme fusionnée
