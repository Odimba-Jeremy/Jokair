...
def init_workflow_tables():
    ...
    # Créer la table medical_boxes si elle n'existe pas
    try:
        supabase.table("medical_boxes").select("*").limit(1).execute()
    except Exception:
        try:
            # SQL pour créer la table via l'API
            ...
            print(" Table medical_boxes créée avec un box par défaut")
        except Exception as e:
            print(f" Création medical_boxes: {str(e)[:100]}")
